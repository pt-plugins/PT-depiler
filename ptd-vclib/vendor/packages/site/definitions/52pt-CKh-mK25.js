import s, {
  CategoryIncldead as p,
  CategorySpstate as d,
  CategoryInclbookmarked as u,
  SchemaMetadata as v,
} from "../schemas/NexusPHP-BNC4SlPA.js";
import { userInfoWithInvitesInUserDetailsPage as g } from "./kunlun-CkdsklM5.js";
...
class R extends s {
  guessUserLevelId(n) {
    if (!n.levelName) return 0;
    const a = n.levelName.replace(/[\s _]+/g, "").toLowerCase(),
      m = o.levelRequirements?.find((e) =>
        [e.name, ...(e.nameAka ?? [])].some((i) => i.replace(/[\s _]+/g, "").toLowerCase() === a),
      );
    if (m) return m.id;
    const t = o.levelRequirements?.find((e) =>
      [e.name, ...(e.nameAka ?? [])].some((i) => {
        const r = i.replace(/[\s _]+/g, "").toLowerCase();
        return (a === "校长" && r.includes("名誉校长")) || (a === "名誉校长" && r === "校长") ? !1 : r.includes(a);
      }),
    );
    return t ? t.id : super.guessUserLevelId(n);
  }
}
export { R as default, o as siteMetadata };
