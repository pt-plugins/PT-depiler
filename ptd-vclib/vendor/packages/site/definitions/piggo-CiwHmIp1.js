import { SchemaMetadata as n, parseSectionedHitAndRunElement as i } from "../schemas/NexusPHP-BNC4SlPA.js";
...
const x = {
  ...
  userInfo: {
    ...n.userInfo,
    selectors: {
      ...n.userInfo.selectors,
      messageCount: {
        text: 0,
        selector: "div.message-alerts-container",
        elementProcess: (e) => {
          let o = 0;
          const t = e.querySelectorAll("div.message-alert");
          for (const r of t) {
            const a = r.textContent?.match(/(\d+)/);
            o += a && a.length >= 2 ? parseInt(a[1], 10) : 0;
          }
          return o;
        },
      },
      hnrPreWarning: {
        ...
        elementProcess: (e) => i(e)?.hnrPreWarning ?? 0,
      },
      hnrUnsatisfied: {
        text: 0,
        selector: ["#info_block a[href*='myhr.php']:last"],
        elementProcess: (e) => i(e)?.hnrUnsatisfied ?? 0,
      },
    },
  },
  levelRequirements: [
    { id: 1, name: "乳猪", privilege: "新用户的默认级别。无法直接发布种子，需添加候选后等待审核。" },
    {
      id: 2,
      name: "烤乳猪",
      downgrade:
