import { m as p } from "../../../es-toolkit/toMerged-Be-qf92q.js";
import { E as d } from "../types/torrent-BvvY2NbA.js";
import { K as c } from "../index-COeZNva1.js";
import { p as v } from "../utils/filesize-D_1hx4u8.js";
import g, {
  SchemaMetadata as s,
  CategoryIncldead as h,
  CategorySpstate as y,
  CategoryInclbookmarked as S,
...
const f = {
      ...
      invites: {
        ...
        filters: [
          (e) => {
            if (!e?.trim()) return 0;
            try {
              const a = e.match(/Available:\s*(\d+)/i);
              if (a) return parseInt(a[1], 10) || 0;
              const n = parseInt(e.match(/\d+/)?.[0] || "0", 10);
              return isNaN(n) ? 0 : n;
            } catch {
              return 0;
            }
    ...
    process: [
      ...s.userInfo.process.map((e) =>
        e.requestConfig.url === "/userdetails.php" ? { ...e, fields: [...(e.fields || []), "invites"] } : e,
      ),
    ],
  },
  H = {
      ...
      selectors: {
        ...
        progress: {
          ...
          filters: [
            (e) => {
              switch (!0) {
                case /snatched_(no|yes)_yes/.test(e):
                  return 100;
                case /snatched_(no|yes)_no/.test(e):
                default:
                  return 0;
              }
            },
    ...
    levelRequirements: [
      { id: 1, name: "User", privilege: "新用户的默认级别。可以同时下载2个种子;可以删除自己上传的字幕." },
      {
        id: 2,
        name: "Power User",
        interval: "P4W",
        ...
        ratio: 1.05,
        privilege:
          '可以查看NFO文档; 可以查看用户列表; 可以请求续种; 可以在求种补种区发主题帖;可以查看友站邀请专区;可以查看排行榜; 可以查看其它用户的种子历史(如果用户隐私等级未设置为"强"); 可以在魔力值系统购买更多邀请名额.可以同时下载5个种子.',
      },
      {
        id: 3,
        name: "Elite User",
      ...
      { id: 204, name: "Coder", groupType: "manager", privilege: "网站的维护开发组成员." },
      {
        ...
        groupType: "manager",
        privilege:
          "可以查看管理组信箱、举报信箱; 管理趣味盒内容、投票内容; 可以编辑或删除任何上传的种子; 可以管理候选;可以管理论坛帖子、用户评论; 可以查看机密日志; 可以删除任何字幕; 可以管理日志中的代码、史册; 可以查看用户的邀请记录;可以管理用户帐号的一般信息. 不能管理友情链接、最近消息、论坛版块; 不能删除账号.",
      },
      {
        id: 206,
        name: "Administrator",
...
class K extends g {
  async parseUserInfoForSeedingStatus(a) {
    const n = a.id,
      i = (await this.request({ url: "/viewusertorrents.php", params: { id: n, show: "seeding" } })).data;
    let r = { seeding: 0, seedingSize: 0 };
    if (i && i?.includes("<table")) {
      const o = c(i),
        t = Array.from(o.querySelectorAll("tr")).slice(1);
      ((r.seeding = t.length),
        t.forEach((u) => {
          const m = u.querySelector("td:nth-child(3)");
          m && (r.seedingSize += v(m.textContent || "0"));
        }));
    }
    return ((a = p(a, r, (o, t) => (typeof t > "u" ? o : t))), a);
  }
  async parseUserInfoForUploads(a) {
    const n = a.id,
      r = (await this.request({ url: "/viewtorrentsdetail.php", params: { id: n } })).data.match(
        /<\/a>\s*[^<]*<div id="ka"/,
      );
    if (r) {
      const t = r[0].match(/(\d+)/);
...
export { K as default, H as siteMetadata };
