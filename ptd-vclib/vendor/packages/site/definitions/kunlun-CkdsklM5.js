      ...
      invites: {
        ...
        filters: [
          (e) => {
            if (!e?.trim()) return 0;
            try {
              const n = e.match(/Available:\s*(\d+)/i);
              if (n) return parseInt(n[1], 10) || 0;
              const t = parseInt(e.match(/\d+/)?.[0] || "0", 10);
              return isNaN(t) ? 0 : t;
            } catch {
              return 0;
            }
...
export { C as siteMetadata, s as userInfoWithInvitesInUserDetailsPage };
