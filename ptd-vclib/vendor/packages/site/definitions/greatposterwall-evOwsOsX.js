...
import d, { SchemaMetadata as i } from "../schemas/GazelleJSONAPI-1mDVuO-b.js";
...
const l = {
  ...
  A = {
    ...
    category: [
      { name: "国语配音", key: "chinesedubbed", options: [{ name: "是", value: "1" }] },
      { name: "特效字幕", key: "specialsub", options: [{ name: "是", value: "1" }] },
      { name: "自制", key: "diy", options: [{ name: "是", value: "2" }] },
      { name: "自购", key: "buy", options: [{ name: "是", value: "2" }] },
      {
        name: "语言",
        key: "language",
        options: o([
      ...
      { name: "片源", key: "source", options: o(["VHS", "DVD", "HD-DVD", "TV", "HDTV", "WEB", "Blu-ray", "Other"]) },
      { name: "编码", key: "codec", options: o(["DivX", "XviD", "x264", "H.264", "x265", "H.265", "Other"]) },
      { name: "容器", key: "container", options: o(["AVI", "MPG", "MP4", "MKV", "VOB IFO", "ISO", "m2ts", "Other"]) },
      {
        name: "分辨率",
        key: "resolution",
        options: o(["NTSC", "PAL", "480p", "576p", "720p", "1080i", "1080p", "2160p", "Other"]),
      },
      { name: "处理", key: "processing", options: o(["Encode", "Remux", "DIY", "Untouched"]) },
      {
        name: "标签",
        key: "taglist",
        options: o([
...
class E extends d {
  async transformGroupTorrent(a, e) {
    const { authkey: m, passkey: p } = await this.getAuthKey(),
      r = [];
    (e.isPersonalFreeleech ? r.push({ name: "Free", color: "blue" }) : l[e.freeType] && r.push(l[e.freeType]),
      e.remasterTitle
        .split("/")
        .map((t) => t.trim())
        .forEach((t) => {
      ...
      e.jinzhuan && r.push({ name: "禁转", color: "deep-orange-darken-1" }));
    const u = [n(a.groupSubName), n(a.groupName)].filter(Boolean).join(" / ");
    return {
      site: this.metadata.id,
      id: e.torrentId,
      title: `${u} [${a.groupYear}] [${a.releaseType}]`,
      subTitle:
        `${e.resolution} / ${e.codec} / ${e.source} / ${e.container}` +
        (e.remasterYear ? ` / ${e.remasterYear}` : "") +
        (e.remasterCustomTitle ? ` / ${n(e.remasterCustomTitle)}` : "") +
        (e.scene ? " / Scene" : ""),
      url: `${this.url}torrents.php?id=${a.groupId}&torrentid=${e.torrentId}`,
      link: `${this.url}torrents.php?action=download&id=${e.torrentId}&authkey=${m}&torrent_pass=${p}`,
      ...
      category: a.releaseType || "",
      tags: r,
      ext_imdb: a.imdbId,
      ext_douban: a.doubanId !== "0" ? a.doubanId : null,
    };
  }
  async getUserInfoResult(a = {}) {
    let e = await super.getUserInfoResult(a);
    return (e.id && (e.trueDownloaded = await this.getUserTrueDownloaded(e.id)), e);
  }
  async getUserTrueDownloaded(a) {
    await this.sleepAction(this.metadata.userInfo?.requestDelay);
    const { data: e } = await this.request({ url: `/user.php?id=${a}`, responseType: "document" }, !0);
    return this.getFieldData(e, this.metadata.userInfo?.selectors?.trueDownloaded) || 0;
  }
}
export { s as attrMap, E as default, A as siteMetadata };
