...
import { G as r, T as o } from "../utils/filesize-D_1hx4u8.js";
const l = {
    1e3: "电影",
    1010: "电影-原盘",
    1015: "电影-无损提取",
  ...
  u = {
    ...
    category: [
      {
        name: "分类",
        key: "c",
        options: Object.entries(l).map(([e, t]) => ({ name: t, value: Number(e) })),
        cross: { mode: "append" },
      },
      {
        name: "筛选",
    ...
    search: {
      ...
      requestConfig: { url: "/browse.php", params: { st: 2 } },
      selectors: {
        rows: { selector: "table.torrenttable > tbody > tr:has(a[href^='details.php?tid='])" },
        category: {
          selector: "a[href*='browse.php?c=']",
          attr: "href",
          filters: [{ name: "querystring", args: ["c"] }, (e) => l[Number(e)] ?? e],
        },
        title: {
          selector: "a[href^='details.php?tid=']",
          elementProcess: (e) => {
            const t = e.parentElement ?? e,
              i = Array.from(e.querySelectorAll("span.t_t"))
                .map((s) => s.innerText.trim())
                .filter(Boolean),
              n = Array.from(t.querySelectorAll("span.t_t_c, span.t_t_c_c"))
                .map((s) => s.innerText.trim())
                .filter(Boolean)
                .join(" "),
              d = !!t.querySelector("span.t_t_d_n"),
              c = !!t.querySelector("span.t_t_d");
            if (d) return i.join("");
            if (c) {
              const s = i.join(" ");
              return n
                ? [n, s]
                    ...
                    .map((a) => (/^\[.*\]$/.test(a) ? a : `[${a}]`))
                    .join("")
                : s || (e.innerText ?? e.textContent ?? "").trim();
            }
        ...
        subTitle: {
          ...
          elementProcess: (e) =>
            Array.from(e.querySelectorAll("span.t_t_d, span.t_t_d_n"))
        ...
        url: { selector: "a[href^='details.php?tid=']", attr: "href" },
        id: {
          selector: "a[href^='details.php?tid=']",
          attr: "href",
          filters: [{ name: "querystring", args: ["tid"] }],
        },
        link: { selector: "a[href^='download.php?tid=']:not([href*='ssl'])", attr: "href" },
        size: {
          selector: "> td:nth-child(4)",
          filters: [(e) => e.trim().replace(/([KMGTPEZ])$/i, "$1B"), { name: "parseSize" }],
        },
        ...
        comments: { selector: "> td:nth-child(3)", filters: [{ name: "parseNumber" }] },
        author: { selector: "> td:nth-child(8) span.o_n a" },
        time: { selector: "> td:nth-child(8) p.add_t", filters: [{ name: "parseTime", args: ["yyyy-MM-dd HH:mm"] }] },
        tags: [
    ...
    noLoginAssert: { matchSelectors: ['a[href*="login_pt.php"]'] },
    userInfo: {
      ...
      process: [
        ...
        },
        {
          ...
          selectors: {
            uploaded: { selector: "td.lefttdud:contains('上传流量') + td", filters: [{ name: "parseSize" }] },
            downloaded: { selector: "td.lefttdud:contains('下载流量') + td", filters: [{ name: "parseSize" }] },
            ratio: { selector: "td.lefttdud:contains('共享率') + td", filters: [{ name: "parseNumber" }] },
            bonus: {
              selector: "td.lefttdud:contains('星辰') + td",
              filters: [
                (e) => {
                  const t = e.match(/累计([\d.]+)/);
                  return t ? parseFloat(t[1]) : 0;
                },
              ],
            },
            levelName: { selector: "td.lefttdud:contains('用户级别') + td" },
            joinTime: { selector: "td.lefttdud:contains('注册时间') + td", filters: [{ name: "parseTime" }] },
            lastAccessAt: {
              selector: "td.lefttdud:contains('最后访问时间(IPv4)') + td",
              filters: [{ name: "parseTime" }],
            },
            invites: { selector: "td.lefttdud:contains('邀请数目') + td", filters: [{ name: "parseNumber" }] },
            uploads: { selector: "td.lefttdud:contains('发种次数') + td", filters: [{ name: "parseNumber" }] },
            seeding: {
            ...
            seedingSize: {
              ...
              filters: [
                (e) => {
                  const t = e.match(/共([\d.]+ ?[TGMKZEP]?i?B)/);
                  return t ? t[1] : "0";
                },
                { name: "parseSize" },
              ],
    ...
    levelRequirements: [
      ...
      { id: 1, groupType: "user", name: "Lv1. 拨号上网", privilege: "新注册用户的默认等级" },
      { id: 2, groupType: "user", name: "Lv2. 十兆宽带", uploaded: 23 * r, downloaded: 10 * r, ratio: 1.3 },
      { id: 3, groupType: "user", name: "Lv3. 百兆宽带", uploaded: 111 * r, downloaded: 10 * r, ratio: 1.55 },
      { id: 4, groupType: "user", name: "Lv4. 千兆宽带", uploaded: 520 * r, downloaded: 15 * r, ratio: 2.05 },
      { id: 5, groupType: "user", name: "Lv5. 校园核心网", uploaded: 1314 * r, downloaded: 15 * r, ratio: 2.3 },
      { id: 6, groupType: "user", name: "Lv6. 城域骨干网", uploaded: 2333 * r, downloaded: 20 * r, ratio: 2.55 },
      { id: 7, groupType: "user", name: "Lv7. 国际互联网", uploaded: 6666 * r, downloaded: 20 * r, ratio: 3.05 },
      {
        id: 8,
        groupType: "user",
        name: "Lv8. 万物互通网",
