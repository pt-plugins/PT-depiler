const a = {
  ...
  search: {
    requestConfig: { url: "/torrents_beta.php", params: { active: 0, options: 0 } },
    keywordPath: "params.search",
    selectors: {
      rows: { selector: "div.torrentsContainer > div.torrent" },
      id: { selector: "a.torrent_link", attr: "href", filters: [(e) => e.match(/id=([a-f0-9]+)/)?.[1] ?? e] },
      ...
      time: {
        ...
        filters: [(e) => e.replace(/^\s*on\s+/i, "").trim(), { name: "parseTime", args: ["HH:mm dd-MMM-yyyy"] }],
      },
      ...
      completed: { selector: "div.downloadTimes a", filters: [{ name: "parseNumber" }] },
    ...
    process: [
      {
        ...
        selectors: {
          ...
          bonus: { selector: ["a[href*='seedbonus']:first"], filters: [{ name: "parseNumber" }] },
          seeding: { selector: ["span[title='Seed']"], filters: [{ name: "parseNumber" }] },
        },
      },
      {
        ...
        selectors: {
          name: { selector: "td:contains('User') + td" },
          levelName: { selector: ["td:contains('Rank') + td"] },
          downloaded: { selector: ["td:contains('Downloaded') + td"], filters: [{ name: "parseSize" }] },
          uploaded: { selector: ["td:contains('Uploaded') + td"], filters: [{ name: "parseSize" }] },
          ratio: { selector: "td:contains('Ratio') + td", filters: [{ name: "parseNumber" }] },
          joinTime: {
            selector: ["td:contains('Joined on') + td"],
            filters: [(e) => (e.includes(" ") ? e.split(" ")[0] : e), { name: "parseTime" }],
          },
        },
      },
    ],
  ...
  levelRequirements: [
    { id: 1, name: "Member", ratio: 0.4 },
    { id: 2, name: "Poweruser", ratio: 1.05, uploaded: "50GB", privilege: "Access to: Online Users, Tracker info" },
    {
      id: 3,
      name: "Extrauser",
      ratio: 2,
