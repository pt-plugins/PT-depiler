import a from "../schemas/AbstractPrivateSite-kkMcHSoo.js";
...
const o = {
  ...
  A = {
    ...
    category: [{ name: "类别", key: "f", options: r(o), cross: { mode: "brackets" } }],
    search: {
      keywordPath: "params.nm",
      requestConfig: { url: "/forum/tracker.php", params: { o: 1, s: 2 }, responseType: "document" },
      advanceKeywordParams: { imdb: !1 },
      selectors: {
        rows: { selector: "table#tor-tbl > tbody > tr:has(a.tr-dl)" },
        category: { selector: "a.f", attr: "href", filters: [{ name: "querystring", args: ["f"] }, (i) => o[i]] },
    ...
    detail: {
      ...
      selectors: { title: { selector: "a#topic-title" }, link: { selector: "a.dl-link", attr: "href" } },
    },
    userInfo: {
      pickLast: ["id"],
      process: [
        ...
        },
        {
          ...
          selectors: {
            ...
            seedingSize: {
              ...
              elementProcess: (i) => {
                const e = i.querySelectorAll("tr > td:nth-child(6) > u");
                let s = 0;
                return (
                  e.forEach((t) => {
                    s += parseInt(t.innerText || t.textContent);
                  }),
                  s
                );
              },
    ...
    levelRequirements: [{ id: 1, name: "User", nameAka: ["Пользователь"] }],
  };
class C extends a {
  async getTorrentDownloadLink(e) {
    return (
      e.link &&
        e.link.includes("/viewtopic.php?t=") &&
        (e.link = e.link.replace(/viewtopic\.php\?t=(\d+)/, "dl.php?t=$1")),
      super.getTorrentDownloadLink(e)
    );
  }
}
export { C as default, A as siteMetadata };
