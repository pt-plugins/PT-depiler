...
import { sleep } from '@/resource/utils/filter';
...
export const serverConfig: GoogleDriveConfig = {
  id: '6ceebdc7-3cd2-4b51-9120-42c0f734dd45',
  name: 'GoogleDrive',
  type: 'GoogleDrive',
...
export default class GoogleDrive implements IBackupServer<GoogleDriveConfig> {
  ...
  private async request <T extends ApiResponse> (config: AxiosRequestConfig = {}, retry: number = 3): Promise<AxiosResponse<T>> {
    ...
    config.headers = { ...config.headers, authorization: 'Bearer ' + accessToken.access_token };
  ...
  private async getParentId (): Promise<string> {
    ...
    if (scope === 'appDataFolder') {
      ...
      if (req.data.files.length === 0) { // 如果没有这个文件夹的话，就创建一个
  ...
  async deleteFile (path: string): Promise<boolean> {
    try {
      await this.request({ method: 'delete', url: urljoin(this.ApiEndpoint, path) });
      return true;
    } catch (e) {
      const response = (e as AxiosError<ErrorResponse>).response!;
  ...
  async list (options: IBackupFileListOption = {}): Promise<IBackupFileInfo[]> {
    ...
    do {
      ...
      const { data } = await this.request<FileListResponse>({ url: this.ApiEndpoint, params });
