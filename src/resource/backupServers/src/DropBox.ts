...
import {
  IBackupConfig, IBackupMetadata, IBackupServer,
  IBackupFileListOption, IBackupFileInfo
} from '../type';
...
export default class DropBox implements IBackupServer<DropBoxConfig> {
  ...
  async deleteFile (path: string): Promise<boolean> {
    try {
      const { data } = await this.request<{'error_summary'?: string, metadata?: any}>({
