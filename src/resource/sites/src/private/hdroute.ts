  ...
  selector: {
    search: {
      ...
      id: {
        ...
        attr: 'id',
        filters: [
          (query: string) => parseInt(query.replace('dl_torrent_', ''))
        ]
      },
      ...
      category: { text: 'ALL' },
      seeders: { selector: '.torrent_count.strong:eq(0)', filters: [(query: string) => query === '---' ? 0 : query] },
      leechers: { selector: '.torrent_count.strong:eq(1)', filters: [(query: string) => query === '---' ? 0 : query] },
      completed: { text: -1 },
      comments: { selector: 'span.torrent_comments_count' },

      tags: [
        { name: 'Free', selector: 'figure.sprite_dlp000, figure.sprite_tempo_free' }, // 把临时Free并入Free中
