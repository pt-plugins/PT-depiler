...
<template>
  <v-row no-gutters class="align-center" style="line-height: 8px">
    <v-col :cols="3">
      <v-icon size="x-small" :icon="progressIcon(prop.torrent.status)" :color="progressColor(prop.torrent.status)"></v-icon>
    </v-col>
    <v-col :cols="9">
      <v-progress-linear v-model="prop.torrent.progress" :color="progressColor(prop.torrent.status)"
                         :title="`${prop.torrent.progress}%`"></v-progress-linear>
    </v-col>
  </v-row>
</template>
