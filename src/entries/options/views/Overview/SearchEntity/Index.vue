...
async function doSearch(flush: boolean = true) {
  ...
  const searchPlan = searchData.value.searchPlan = siteStore.getSolutionPlan(searchData.value.searchPlanKey); // 展开searchPlanKey对应的搜索方案
  ...
  for (const plan of searchPlan.plan) {
    ...
    const v = await site.searchTorrents({
      ...
      extraParams: Object.entries(plan.filters).map(x => ({key: x[0], value:x[1]}))
    });
...
watch(() => route.query, (newParams, oldParams) => {
  console.log('search', newParams, oldParams)

  if (newParams.snapshot) {
    // TODO 处理搜索快照
  } else if (newParams.search != oldParams.search || newParams.plan != oldParams.plan) {
    doSearch();
  }
})
  ...
  <v-card>
    ...
    >
      ...
      <template #item.site="{ item }">
        ...
        <a :href="item.siteUrl" target="_blank" rel="noopener noreferrer nofollow" class="captionText">{{ item.siteName }}</a>
      </template>

      <!-- 标题，副标题（如有），标签（如有），| ， -->
      <template #item.title="{ item }">
        <a :href="item.url" target="_blank" rel="noopener noreferrer nofollow" class="text-body-1 font-weight-medium">{{ item.title }}</a>

        <div v-if="(item.tags && item.tags.length) || item.subTitle">
        <span class="mr-1" v-if="item.tags && item.tags.length">
          <span v-for="(tag, index) in item.tags" :key="index"
                class="tag" :title="tag.name"
      ...
      <!-- 评论数（不需要自定义template） -->
      <!-- 上传时间 -->
      <template #item.time="{ item }">{{ dayjs(item.time).format('YYYY-MM-DD HH:mm') }}</template>
