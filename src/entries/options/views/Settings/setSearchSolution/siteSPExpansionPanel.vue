...
function initSitePreAddMap() {
  ...
  siteCategories.value.forEach((category, index) => {
    const definedSp = (preAddMap.value[category.key] ??= category.cross
      ? []
...
<template>
  <v-col>
    <v-expansion-panels v-if="siteCategories.length > 0" v-model="showPanel" multiple>
      <v-expansion-panel v-for="category in siteCategories" :key="category.key">
        <v-expansion-panel-title>
          ...
          <v-chip
            ...
            :color="
              !category.cross?.mode ||
              (Array.isArray(preAddMap[category.key]) &&
                preAddMap[category.key].length > 0)
                ? 'info'
                : ''
            "
          >
            ...
            <v-row v-if="category.cross?.mode" no-gutters>
              <v-col cols="12">
                <v-checkbox
                  :indeterminate="
                    preAddMap[category.key]?.length > 0 &&
                    checkBtnIndeterminate(category)
                  "
                ...
                >
                  <template #label>
                    ...
                    <p
                      v-if="!checkBtnIndeterminate(category)"
  ...
  <v-col>
    <v-btn color="blue" @click="perAddSiteSearchSolution(site)"> 添加 </v-btn>
  </v-col>
</template>
