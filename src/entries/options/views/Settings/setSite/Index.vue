  ...
  <v-card>
    ...
    >
      ...
      <template #item.name="{ item }">
        <ExpandInfo :main="item.name" :expand="item.aka" :open="false">
          <template #default="{ data, field }">
            <span :title="field === 'main' ? (item.description ?? '') : ''">{{ data }}</span>
          </template>
        </ExpandInfo>
      </template>
      ...
      <template #item.url="{ item }">
        <ExpandInfo
          :main="item.url" :expand="item.legacyUrls"
      ...
      <template #item.tags="{ item }">
        <v-chip
          v-for="tag in item.tags?.slice(0,4) /* 只显示前4个tag */" :key="tag"
          class="mr-1 mb-1"
          label
          size="x-small"
          :title="tag"
      ...
      <template #item.isOffline="{ item }">
        <v-switch
          ...
          color="success"
          hide-details
          @update:model-value="(v) => siteStore.simplePatchSite(item.id, 'isOffline', v)"
        />
      </template>
      <template #item.allowSearch="{ item }">
        <v-switch
          ...
          @update:model-value="(v) => siteStore.simplePatchSite(item.id, 'allowSearch', v)"
        />
      </template>
      <template #item.allowQueryUserInfo="{ item }">
        <v-switch
          ...
          @update:model-value="(v) => siteStore.simplePatchSite(item.id, 'allowQueryUserInfo', v)"
        />
      </template>
      <template #item.showMessageCount="{ item }">
        <v-switch
          ...
          color="success"
          hide-details
          :disabled="item.isOffline || !Object.hasOwn(item,'userInfo') || item.allowQueryUserInfo === false"
          @update:model-value="(v) => siteStore.simplePatchSite(item.id, 'showMessageCount', v)"
        />
      </template>
