...
import GazelleJSONAPI, { SchemaMetadata, userJsonResponse, groupBrowseResult, groupTorrent } from "../schemas/GazelleJSONAPI.ts";
...
export const siteMetadata: ISiteMetadata = {
  ...
  userInfo: {
    ...
    selectors:{
      ...
      seedingSize: {
        selector: ["#bprates_overview tbody tr td:nth-child(2)"],
        filters: [{ name: "parseSize" }]
      },
      bonusPerHour: {
        selector: ["#bprates_overview tbody tr td:nth-child(3)"],
...
export default class DICMusic extends GazelleJSONAPI {
  protected override async getUserExtendInfo(userId: number): Promise<Partial<IUserInfo>> {
    ...
    return this.getFieldsData(
      ...
      ["joinTime", "seeding", "uploads", "uniqueGroups", "perfectFlacs"] as (keyof IUserInfo)[]
  ...
  protected override async getUserSeedingTorrents(userId: number): Promise<Partial<IUserInfo>> {
    ...
    return this.getFieldsData(
      dataDocument,
      this.metadata.userInfo!.selectors!,
      ["bonus", "bonusPerHour", "seedingSize"] as (keyof IUserInfo)[]
    ) as Partial<IUserInfo>;
  }
