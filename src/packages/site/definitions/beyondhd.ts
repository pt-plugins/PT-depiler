...
import Unit3D from "../schemas/Unit3D";
...
export const siteMetadata: ISiteMetadata = {
  ...
  search: {
    ...
    selectors: {
      rows: {
        selector: "div.table-torrents > table:last > tbody > tr.librarytab",
      },
      ...
      category: {
        selector: 'a[href*="/categories/"] > div > i[title]',
        attr: "title",
      },
...
export default class beyondhd extends Unit3D {
  private async getUserInfoPathFromSite(): Promise<`${string}.${string}`> {
    ...
    return this.getFieldData(indexDocument, {
      ...
      attr: "href",
      filters: [(query: string) => query.replace(/\/settings.+$/, "")],
    }) as `${string}.${string}`;
  }
