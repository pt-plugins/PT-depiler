/**
 * TODO: search method should change due to https://github.com/ronggang/PT-Plugin-Plus/issues/996
 */
...
export const SchemaMetadata: Partial<ISiteMetadata> = {
  search: {
    ...
    selectors: {
      rows: {
        selector: 'table.table:first > tbody > tr:has(a[href*="/torrent/"])',
      },
      id: {
      ...
      title: {
        selector: "a.torrent-filename",
        attr: "title",
        filters: [(query: string) => query.replace("Torrent Details - ", "")],
      },
      ...
      author: {
        selector: "span.pull-right:has( > i.fa-upload) > span.badge-user",
      },
      category: {
        selector: "> td:nth-child(1) > i",
        attr: "title",
        filters: [(query: string) => query.replace(" Torrent", "")],
      },
...
export default class AvistaZ extends PrivateSite {
  public override async getUserInfo(
    ...
    if (baseUserInfo.name && !baseUserInfo.seedingSize) {
      ...
      // 生成页面信息
      for (
        const pageInfo = { count: 1, current: 1 };
        ...
        if (pageInfo.count === 1) {
          pageInfo.count = this.getFieldData(TListDocument, {
            text: 1,
            selector:
              'ul.pagination:first > li:has(a[href*="/active?page="]):nth-last-child(2) > a',
            attr: "href",
            filters: [
              (query: string) =>
