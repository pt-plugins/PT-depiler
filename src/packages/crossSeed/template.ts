/**
 * download_page 模板渲染（A 路线兜底，跨源通用）。
 * 变量语义见 docs/iyuu-integration-plan.md §4（源码对应 DriverPthome::parseReplace 等）。
 */
import type { ICrossSeedTemplateSite, IYUUDownloadCredentials } from "./types";
...
/** 由模板站点条目标线程协议前缀（is_https: 0=http、1=https、2=http(s)） */
export function siteTemplateProtocol(templateSite: { is_https: 0 | 1 | 2 }): "http://" | "https://" {
  return templateSite.is_https === 0 ? "http://" : "https://";
}

/** 兼容旧导出名（@ptd/iyuu 时代） */
export { siteTemplateProtocol as iyuuProtocol };
