...
browser.storage.local.get('PTPP-v2-Data').then(initPersistData => {
  if (initPersistData['PTPP-v2-Data']) { // 如果有值，则使用persist data替换整个store
    store.$patch(initPersistData['PTPP-v2-Data']);
  }
  store.isReady = true;
});

// 当store出现任何变更的时候，更新浏览器数据
// 这样我们就不用在每个action中写方法了
store.$subscribe((mutation, state) => {
  console.log('Store subscribe ', mutation, state);
  browser.storage.local.set({ 'PTPP-v2-Data': JSON.parse(JSON.stringify(state)) });
});
